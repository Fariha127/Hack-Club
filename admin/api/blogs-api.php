...
switch ($action) {
    ...
    case 'update':
        ...
        if ($action === 'create') {
            $st = $db->prepare("INSERT INTO blogs (title, slug, author_name, author_email, author_university, author_department, content, excerpt, cover_image, tags, status, published_at, reviewed_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $st->execute([
                $title, $slug, $input['author_name'], $input['author_email'] ?? null,
                $input['author_university'] ?? null, $input['author_department'] ?? null,
                $input['content'], $input['excerpt'] ?? null, $imagePath,
                $input['tags'] ?? null, $status, $publishedAt, $adminId
            ]);
            json_ok(['id' => $db->lastInsertId(), 'message' => 'Blog created.']);
        } else {
            $blog = $db->prepare("SELECT cover_image, published_at FROM blogs WHERE id = ?");
            $blog->execute([$id]);
            $old = $blog->fetch();
            ...
            $st = $db->prepare("UPDATE blogs SET title=?,slug=?,author_name=?,author_email=?,author_university=?,author_department=?,content=?,excerpt=?,cover_image=?,tags=?,status=?,published_at=?,reviewed_by=? WHERE id=?");
            $st->execute([
                $title, $slug, $input['author_name'], $input['author_email'] ?? null,
                $input['author_university'] ?? null, $input['author_department'] ?? null,
                $input['content'], $input['excerpt'] ?? null, $img,
