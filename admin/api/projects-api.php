...
switch ($action) {
    ...
    case 'update':
        ...
        if ($action === 'create') {
            $db->prepare("INSERT INTO projects (title, slug, description, cover_image, team_members, mentors, technologies, project_status, display_order, created_by) VALUES (?,?,?,?,?,?,?,?,?,?)")
               ->execute([$title, $slug, $input['description'], $imagePath, $input['team_members'] ?? null, $input['mentors'] ?? null, $input['technologies'] ?? null, $status, $order, $adminId]);
            ...
            $db->prepare("UPDATE projects SET title=?,slug=?,description=?,cover_image=?,team_members=?,mentors=?,technologies=?,project_status=?,display_order=? WHERE id=?")
               ->execute([$title, $slug, $input['description'], $image, $input['team_members'] ?? null, $input['mentors'] ?? null, $input['technologies'] ?? null, $status, $order, $id]);
